...
# type: ignore
"""
Utilities for exporting ALL data from the pilot study to an xlsx.
"""
...
def main():
    ...
    with database.SessionFactory() as session:
        ...
        logger.info(
            f"removed {len(all_attempts) - len(attempts)} attempts ({len(attempts)} remaining)"
        )
        ...
        a1_df = pd.DataFrame([attempt_to_dict(x, ALLOWED_EMAILS) for x in a1_attempts])
        a2_df = pd.DataFrame([attempt_to_dict(x, ALLOWED_EMAILS) for x in a2_attempts])

        # sort by user_id and then attempt_date (oldest first)
        a1_df = a1_df.sort_values(
            by=["user_consent", "user_id", "attempt_date"],
            ascending=[False, True, True],
        )
        a2_df = a2_df.sort_values(
            by=["user_consent", "user_id", "attempt_date"],
            ascending=[False, True, True],
        )
...
## for each assignment:
def attempt_to_dict(att: models.Attempt, allowed_emails: set[str]) -> dict:
    ...
    hf_data = FeedbackData(**hf.data) if hf is not None else None
    ...
    # print date in Amsterdam timezone (UTC+2)
    # date_str = att.created_at.strftime("%Y-%m-%d %H:%M:%S")
    amsterdam_tz = pytz.timezone("Europe/Amsterdam")

    obj = {
        "user_id": att.user.id,
        "user_consent": att.user.email in allowed_emails,
        "attempt_id": att.id,
        "attempt_date": att.created_at.astimezone(amsterdam_tz).strftime(
            "%Y-%m-%d %H:%M"
        ),
        "attempt_goal": smart_data.goal,
        "attempt_plan": smart_data.plan,
        "ai_feedback": af_data.feedback if af_data is not None else None,
        "human_feedback": safe_get(hf_data, "feedback"),
        "approved": safe_get(hf_data, "approved"),
        "review_rating": hf_data.eval_metrics.rating
        if safe_get(hf_data, "eval_metrics")
        else None,
...
def _get_skip_emails(fname: str = "pilot_skip_emails.txt") -> set[str]:
    """Get  optional list of test student account emails to skip."""
    skip_emails = {}
    if not os.path.isfile(fname):
        logger.warning(f"file '{fname}' not found, continuing without skip list")
    else:
        skip_emails = _read_emails(fname)
    logger.info(f"skip emails: {len(skip_emails)}")
    return skip_emails
